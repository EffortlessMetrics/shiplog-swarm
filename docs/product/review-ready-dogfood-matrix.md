...
| 4 | Repaired manual-only packet | Manual repair can improve the packet, rerun intake, and hand off to `repair diff` before planning again (#343, #344, #388). Packet readiness, claim candidates, missing-context prompts, and share posture now have report-derived alignment proof (#394). | Confirm `runs diff` shows quality movement without overstating source-backed confidence for manual-only evidence. |
