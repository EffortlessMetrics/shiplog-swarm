...
| --- | --- | --- | --- | --- |
| 1 | Empty directory | Missing config is modeled as setup work with `start --yes` as the explicit write-producing next action (#402, #403). `start --yes` reuses the guided `shiplog.toml` and `manual_events.yaml` scaffold; `start --dry-run` previews it without writing. The guided setup transcript still proves the underlying init writer while doctor, doctor JSON, and sources status create no output artifacts (#418). | Future wording passes should keep the exact first-run text aligned with the transcript. | Empty directories are allowed to start with confirmed setup writes; doctor itself remains read-only. |
| 2 | Git repo with no config | `doctor --setup` reports missing config instead of trying intake, and `start --yes` enables local Git when the current directory is a readable repo (#402, #403, #405). The lower-level `shiplog init --guided` command remains an alternative scaffold path. | Keep proof that a no-config Git repo does not trigger provider checks or packet claims before setup exists. | The first useful command is `start --yes`, which writes local setup files after confirmation. |
