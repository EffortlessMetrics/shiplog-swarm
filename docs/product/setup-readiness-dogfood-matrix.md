...
| 2 | Git repo with no config | `doctor --setup` reports missing config instead of trying intake, and `start --yes`/`init --guided` enables local Git when the current directory is a readable repo (#402, #403, #405). | Keep proof that a no-config Git repo does not trigger provider checks or packet claims before setup exists. | The first useful command is `start --yes`, which writes local setup files after confirmation. |
