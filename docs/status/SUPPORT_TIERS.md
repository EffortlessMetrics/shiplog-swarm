...
| Repo contract report | Stabilizing | Humans and agents can inspect the current source-of-truth graph, source/swarm topology, branch hygiene, branch-protection contract, and promotion receipt freshness from generated JSON and Markdown. | `rtk cargo xtask repo-contract-report` | Writes `target/source-of-truth/graph.json` and `target/source-of-truth/graph.md`; classifies source-ahead promotion merges, splits remote branch cleanup candidates into merged/review buckets, checks whether `shiplog-swarm/main` requires only `Shiplog Rust Small Result` when GitHub inspection is available, and reports missing latest promotion receipts; derived report only, not a branch-protection mutator, validator, or branch cleanup command. |
