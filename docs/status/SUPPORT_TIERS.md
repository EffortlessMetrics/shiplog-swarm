...
| Repo contract report | Stabilizing | Humans and agents can inspect the current source-of-truth graph, source/swarm topology, branch hygiene, PR/issue queue hygiene, branch-protection contract, and promotion receipt freshness from generated JSON and Markdown. | `rtk cargo xtask repo-contract-report` | Writes `target/source-of-truth/graph.json` and `target/source-of-truth/graph.md`; classifies source-ahead promotion merges, splits remote branch cleanup candidates into merged/review buckets, reports open source/swarm PRs and issues when GitHub inspection is available, checks whether `shiplog-swarm/main` requires only `Shiplog Rust Small Result`, and reports missing latest promotion receipts; derived report only, not a branch-protection mutator, validator, PR/issue mutator, or branch cleanup command. |
