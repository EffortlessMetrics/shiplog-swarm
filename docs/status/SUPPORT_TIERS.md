...
| Repo contract report | Stabilizing | Humans and agents can inspect the current source-of-truth graph, source/swarm topology, branch hygiene, and promotion receipt freshness from generated JSON and Markdown. | `rtk cargo xtask repo-contract-report` | Writes `target/source-of-truth/graph.json` and `target/source-of-truth/graph.md`; classifies source-ahead promotion merges, splits remote branch cleanup candidates into merged/review buckets, and reports missing latest promotion receipts; derived report only, not a validator or branch cleanup command. |
