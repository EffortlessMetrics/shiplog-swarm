...
| Promotion body generator | Stabilizing | Agents can draft source promotion PR bodies with the swarm head, included swarm PRs, proof slots, and merge-commit boundary. | `rtk cargo xtask promotion-body --output target/source-of-truth/promotion-body.md` | Writes `target/source-of-truth/promotion-body.md`; derived draft only, no GitHub API calls, PR creation, PR editing, branch mutation, merge, release, or publish action. |
