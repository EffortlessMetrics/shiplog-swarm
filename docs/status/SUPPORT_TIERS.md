...
| Repo contract report | Stabilizing | Humans and agents can inspect the current source-of-truth graph and source/swarm topology from generated JSON and Markdown. | `cargo xtask repo-contract-report` | Writes `target/source-of-truth/graph.json` and `target/source-of-truth/graph.md`; classifies source-ahead promotion merge commits separately from source-only drift; derived report only, not a validator. |
