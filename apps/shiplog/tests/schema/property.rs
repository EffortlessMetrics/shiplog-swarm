...
proptest! {
    ...
    fn event_id_deterministic(
        ...
        let id1 = shiplog::ids::EventId::from_parts(parts.iter().map(|s| s.as_str()));
        let id2 = shiplog::ids::EventId::from_parts(parts.iter().map(|s| s.as_str()));
    ...
    fn event_id_different_inputs_differ(
        ...
        let id_a = shiplog::ids::EventId::from_parts(parts_a.iter().map(|s| s.as_str()));
        let id_b = shiplog::ids::EventId::from_parts(parts_b.iter().map(|s| s.as_str()));
...
fn completeness_all_variants_roundtrip() {
    use shiplog::schema::coverage::Completeness;
    let variants = [
